...
const createApp = () => {
  ...
  app.post("/projects", async (req, res) => {
    try {
      ...
      if (!allowedUrgency.includes(urgency)) {
        return res
          .status(400)
          .json({ error: "Urgency must be Niedrig, Mittel, or Hoch" });
      }
      ...
      if (colorId !== undefined && colorId !== null && colorId !== "") {
        ...
        const colorRow = await get(
          `SELECT id FROM filament_colors WHERE id = ?`,
          [parsedColorId]
        );
        if (!colorRow) {
    ...
    try {
      ...
      if (status) {
        ...
        if (!allowedStatus.includes(status)) {
          return res
            .status(400)
            .json({ error: "Status must be Offen, In Arbeit, or Fertig" });
        }
        updates.push("status = ?");
        params.push(status);
      }
  ...
  return { app, publishProjectStats };
};

const startServer = async () => {
  await init();
  const { app, publishProjectStats } = createApp();
  await publishProjectStats();
  app.listen(PORT, () => {
